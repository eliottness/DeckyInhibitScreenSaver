...
**桌面模式兼容性：** 本插件使用 `ALLOW_REPLACEMENT` 标志注册 D-Bus 服务，这允许 KDE 在切换到桌面模式时无缝接管服务名称。此标志告诉 D-Bus 其他应用程序（如 KDE Plasma）可以替换我们对这些服务的所有权而不会发生冲突。当切换回游戏模式并且 KDE 释放名称时，插件会自动重新注册它们。这提供了模式之间干净、无冲突的过渡。
